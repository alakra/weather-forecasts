...
describe NDFD::Client::QueryProperty do
  let(:name) { :thing }
  let(:klass) { NDFD::Client::QueryProperty }
  ...
  describe "#valid?" do
    it "raises an error on failure to validate required values" do
      ...
      expect { instance.valid?({}) }.to raise_error(NDFD::Client::RequiredPropertyError)
    end

    it "raises an error on failure to validate matching types" do
      instance = klass.new(name, :type => String)
      expect { instance.valid?(name => 5) }.to raise_error(NDFD::Client::InvalidPropertyTypeError)
    end

    it "raises an error on failure to validate required keys" do
      instance = klass.new(name, :type => Array, :required_keys => [:lat, :long])
      expect { instance.valid?(name => []) }.to raise_error(NDFD::Client::RequiredKeysMissingError)
    end
    ...
    it "raises an error on failure to validate the correct use of a set of options" do
      ...
      expect { instance.valid?(name => "three") }.to raise_error(NDFD::Client::InvalidOptionSpecifiedError)
    end
  end
end
